...
AmqpToHttpBridge.prototype.incoming = function (context) {
    ...
    for (var key in context.message.application_properties) {
        if (key === 'skupper-site') {
            site = context.message.application_properties[key];
        } else {
	...
	response.on('end', function () {
            ...
            metrics.http_egress_request(self.metrics, self.host, options.method, response.statusCode, site, context.message.body.length, message_out.body.length, elapsed(start));
        });
    });
...
HttpToAmqpBridge.prototype.request = function (request, response) {
    ...
    request.on('end', function () {
        ...
        self.client.request(message_out).then(function (message_in) {
            ...
            metrics.http_ingress_request(self.metrics, client_ip(request), request.method, response.statusCode, message_out.body.length, message_in.body.length, elapsed(start));
        }).catch(function (error) {
            console.error(error);
            response.statusCode = 500;
            response.end(error.toString());
            metrics.http_ingress_request(self.metrics, client_ip(request), request.method, response.statusCode, message_out.body.length, error.toString().length, elapsed(start));
        });
    });
};
