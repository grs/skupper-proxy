...
AmqpToHttpBridge.prototype.incoming = function (context) {
    ...
    if (this.hostOverride) {
        headers['Host'] = this.hostOverride
    }
	...
	response.on('end', function () {
            ...
            var bytes_in = context.message.body ? context.message.body.length : 0;
            var bytes_out = message_out.body ? message_out.body.length : 0;
            metrics.http_egress_request(self.metrics, self.host, options.method, response.statusCode, site, bytes_in, bytes_out, elapsed(start));
        });
    });
...
TcpToAmqpBridge.prototype.incoming_connection = function (socket) {
    ...
    socket.on('end', function () {
        ...
        metrics.tcp_ingress_connection_close(self.metrics, conn_id);
        client.close();
    });
};
...
module.exports.udp_to_amqp = function (port, address) {
    return udp.ingress(port, address);
};

module.exports.amqp_to_udp = function (address, host, port) {
    return udp.egress(address, host, port);
};
