...
AmqpToHttpBridge.prototype.incoming = function (context) {
    ...
    var request = http.request(options, function (response) {
        log.info('%s: %s', url.path, response.statusCode);
        var message_out = {
...
HttpToAmqpBridge.prototype.request = function (request, response) {
    ...
    request.on('end', function () {
        ...
        self.client.request(message_out).then(function (message_in) {
            ...
            metrics.http_ingress_request(self.metrics, client_ip(request), request.method, response.statusCode, site, message_out.body ? message_out.body.length : 0, message_in.body ? message_in.body.length: 0, elapsed(start));
        }).catch(function (error) {
            console.error(error);
            response.statusCode = 500;
            response.end(error.toString());
            metrics.http_ingress_request(self.metrics, client_ip(request), request.method, response.statusCode, site, message_out.body ? message_out.body.length : 0, error.toString().length, elapsed(start));
        });
    });
};
