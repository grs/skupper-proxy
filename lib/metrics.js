...
BridgeMetricsAgent.prototype.on_message = function (context) {
    ...
    if (context.message.subject === 'bridge-metrics-request') {
        this.sender.send({to: context.message.reply_to, subject: 'bridge-metrics', correlation_id: process.env.HOSTNAME, body: JSON.stringify(this.metrics)});
    ...
    return function (podsummary) {
        for (var key in labels) {
            if (!(podsummary.labels[key] && (labels[key] === null || labels[key] === podsummary.labels[key]))) {
                log.info('pod with labels %j does not match selector %j on %s', podsummary.labels, labels, key);
                return false;
            }
        }
        return true;
...
SiteMetricsAgent.prototype.on_message = function (context) {
    ...
    if (context.message.subject === 'bridge-metrics') {
        this.inprogress_bridge_metrics_request.add_metrics(context.message.correlation_id, JSON.parse(context.message.body));
        log.info('site metrics agent handled bridge-metrics response from %j', context.message.correlation_id);
    } else if (context.message.subject === 'site-metrics') {
        this.inprogress_site_metrics_request.add_metrics(context.message.correlation_id, JSON.parse(context.message.body));
        ...
        this.wait_until_ready().then(function () {
            self.collect_proxy_metrics().then(function (results) {
                ...
                context.connection.send({to:context.message.reply_to, correlation_id:self.site_id, subject:'site-metrics', body:JSON.stringify(site_metrics)});
...
SiteMetricsAgent.prototype.collect_proxy_metrics = function () {
    ...
    this.local_sender.send({subject: 'bridge-metrics-request', reply_to:this.address});
    log.info('site metrics agent sent bridge-metrics-request');
    //wait for all the results
    var self = this;
    return new Promise(function (resolve, reject) {
        self.inprogress_bridge_metrics_request = new AggregatedBridgeMetrics(self.proxies, self.ip_lookup, resolve, reject);
    });
};
...
SiteMetricsAgent.prototype.collect = function () {
    ...
    return this.wait_until_ready().then(function () {
        //send out request, wait for responses
        self.global_sender.send({subject: 'site-metrics-request', reply_to:self.address});
        ...
        return self.get_site_ids().then(function (ids) {
            if (ids.length === 0) {
                ...
                return new Promise(function (resolve, reject) {
                    self.inprogress_site_metrics_request = new AggregatedSiteMetrics(ids, resolve, reject);
                });
            }
        });
    });
